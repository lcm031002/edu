...
angular.module('ework-ui').controller(
		...
		[ '$scope', '$log', '$state', '$cookieStore', '$rootScope',
				'OrgService', 'PostService',
				'EmployeeExtService', 'erp_employeeMgrService','erp_organizationService', '$uibMsgbox',
			hrmEmployeeInfoAddCtrl ]);

function hrmEmployeeInfoAddCtrl($scope, $log, $state, $cookieStore, $rootScope,
		OrgService, PostService, EmployeeExtService,
		erp_employeeMgrService,erp_organizationService,$uibMsgbox) {
	// 员工列表
	$scope.employeeList = {};
...
//				queryEmployeeInfo();
	...
	$scope.addEmployee = function() {
        ...
        erp_employeeMgrService.add($scope.selectedEmp, function(resp) {
            if (resp.error == false) {
                $uibMsgbox.alert("添加成功！")
            } else {
                $uibMsgbox.alert("添加失败！失败信息：" + resp.message);
            }
        })
	};
	/**
