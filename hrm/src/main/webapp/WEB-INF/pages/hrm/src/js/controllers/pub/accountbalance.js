...
angular.module('ework-ui').
    ...
    .controller('AccountBalanceCtrl', ['$scope', '$log','$state','CompanyService','BranchsService','ReportService',AccountBalanceCtrl]);

function AccountBalanceCtrl($scope, $log,$state,CompanyService,BranchsService,ReportService) {
    $scope.companys = [];
    $scope.reportData = [];
   ...
   $scope.initNumber = function(n){
    ...
    function queryReportData(){
        ...
        param.reportType = "queryAccountBalance";
		...
		$scope.exp = function(){
			...
			if(confirm("确定要导出?")){
				...
				window.open("excels/output/outExcelAccountbalance" + paramUrl, "_blank");
			}
	 }
