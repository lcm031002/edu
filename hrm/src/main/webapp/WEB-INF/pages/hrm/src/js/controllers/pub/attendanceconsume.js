...
angular.module('ework-ui').
    ...
    .controller('AttendanceConsumeCtrl', ['$scope', '$log','$state','CompanyService','BranchsService','ReportService',AttendanceConsumeCtrl]);

function AttendanceConsumeCtrl($scope, $log,$state,CompanyService,BranchsService,ReportService) {
    $scope.companys = [];
    $scope.reportData = [];
    $scope.st=new Date();
    $scope.dt=new Date();
   ...
   $scope.initNumber = function(n){
    ...
    function queryReportData(){
        ...
        param.reportType = "queryAttendanceConsume";
		...
		param.isCourseTime = $('#isCourseTime').prop('checked');
        $scope.isLoadingOrders = 'loading...';
        $scope.reportData = [];
        ReportService.get(param,function(resp){
	...
	$scope.exp = function(){
		...
		if(confirm("确定要导出?")){
			var paramUrl = "?companyId=" + $scope.selectedCompany.id
						...
						+ "&isCourseTime=" +$('#isCourseTime').prop('checked')
						+ "&calcBeginDate=" + $('#beginDate').val() 
						+ "&calcEndDate=" + $('#endDate').val() 
			window.open("excels/output/outExcelAttendanceconsume" + paramUrl, "_blank");
		}
   }
