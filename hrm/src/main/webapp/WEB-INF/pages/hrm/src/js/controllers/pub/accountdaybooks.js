...
angular.module('ework-ui').
    ...
    .controller('AccountDayBooksCtrl', ['$scope', '$log','$state','CompanyService','BranchsService','ReportService',AccountDayBooksCtrl]);

function AccountDayBooksCtrl($scope, $log,$state,CompanyService,BranchsService,ReportService) {
    $scope.companys = [];
    $scope.reportData = [];
    $scope.st=new Date();
    $scope.dt=new Date();
   ...
   $scope.initNumber = function(n){
    ...
    function queryReportData(){
		...
		param.student = $scope.student;
		param.calcBeginDate = $('#beginDate').val();
		param.calcEndDate = $('#endDate').val();
		
        param.reportType = "queryAccountDaybooks";
	...
	$scope.exp = function(){
		...
		if(confirm("确定要导出?")){
			...
			window.open("excels/output/outExcelAccountdaybooks" + paramUrl, "_blank");
		}
    }
