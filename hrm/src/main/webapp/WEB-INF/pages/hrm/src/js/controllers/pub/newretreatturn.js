...
angular.module('ework-ui').
    ...
    .controller('newRetreatTurnCtrl', ['$scope', '$log','$state','CompanyService','BranchsService','ReportService',newRetreatTurnCtrl]);

function newRetreatTurnCtrl($scope, $log,$state,CompanyService,BranchsService,ReportService) {
    $scope.companys = [];
    $scope.reportData = [];
    $scope.st=new Date();
    $scope.dt=new Date();
   ...
   $scope.initNumber = function(n){
    ...
    function queryReportData(){
        ...
        param.reportType = "queryNewRetreatTurn";
	...
	$scope.exp = function(){
		...
		if(confirm("确定要导出?")){
			...
			window.open("excels/output/outExcelNewRetreatTurn" + paramUrl, "_blank");
		}
   }
