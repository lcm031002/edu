...
angular.module('ework-ui').
    ...
    .controller('RechargeAndEnchashmentCtrl', ['$scope', '$log','$state','CompanyService','BranchsService','ReportService',RechargeAndEnchashmentCtrl]);

function RechargeAndEnchashmentCtrl($scope, $log,$state,CompanyService,BranchsService,ReportService) {
    $scope.companys = [];
    $scope.reportData = [];
    $scope.st=new Date();
    $scope.dt=new Date();
   ...
   $scope.initNumber = function(n){
    ...
    function queryReportData(){
        ...
        param.reportType = "queryRechargeAndEnchashment";
	...
	$scope.exp = function(){
		...
		if(confirm("确定要导出?")){
			...
			window.open("excels/output/outExcelRechargeandenchashment" + paramUrl, "_blank");
		}
   }
    queryCompanys();
