...
public class TeacherGroupAttendanceController {
	
	private static final Logger log = Logger.getLogger(TeacherGroupAttendanceController.class);

	@Resource(name = "teacherGroupAttendanceService")
	private TeacherGroupAttendanceService teacherGroupAttendanceService;
    
    @Autowired
    private UserPermissionService userPermissionService;

	@RequestMapping(value = { "/teachergroup/attendance" }, method = RequestMethod.GET, headers = { "Accept=application/json" })
	@ResponseBody
	public Map<String, Object> queryReport(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<TeacherGroupAttendanceReport> queryResult = teacherGroupAttendanceService.queryReport(queryParam);
	...
	@RequestMapping(value = { "/teachergroup/attendance/output" }, method = RequestMethod.GET, headers = { "Accept=application/json" })
	@ResponseBody
	public Map<String, Object> outputReport(HttpServletRequest request, HttpServletResponse response) {
		...
		// 模板文件名
		String templateFileName = "teachergroup_attendance.xlsx";
		// 临时文件名
		String tempFileName = "培英班教研组统计表_"
		...
		try {
			...
			List<TeacherGroupAttendanceReport> queryResult = teacherGroupAttendanceService.queryReport(paramMap);
